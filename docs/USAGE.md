...
- 영한 뜻 채우기는 부정확할 수 있는 무료 보조 기능입니다. 기본적으로 꺼져 있으며, provider를 직접 켠 경우에만 외부 번역 요청을 보냅니다.
...
8. 선택 사항: 부정확할 수 있는 무료 번역 보조 기능을 켠 뒤 item 또는 생성된 `단어장 (...)` 노트에서 `Vocab Flow` -> `빈 Korean meanings 채우기...`를 실행하면 빈 뜻 칸만 채웁니다.
